...
A **Project** is a bounded multi-repo effort under a Product — *a promoted tag with a version-controlled home*: `<product>/projects/<name>/project.yaml` holds its `name`, `state` (`open`/`done`), its `tag`, and an optional `tep` ("why"-TEP ref). A project's **members are not stored** — they're derived: any item (spec/TEP/slice, on any board) carrying the project's `tag` is a member. **`list_projects`** returns every product's projects; **`get_project`** (by `product` + `id`) returns the manifest plus its tag-resolved members `{ board, handle, kind }` across all boards. In the navigator, Projects appear under their Product; **selecting a Project lists its members** (the tagged items across boards) in the **Project Members** view. _(The navigator Product/Project tree and the "New Product"/"New Project" commands land in a follow-up; this is the data + discovery layer.)_
