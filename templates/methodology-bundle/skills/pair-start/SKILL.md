...
2. **Require the Spec's worktree (open/redirect if you're in the canonical checkout).** A Spec runs in its own `spec/SP-{n}` **worktree** (TEP-0008), so work must happen there, never in the canonical/main checkout where it could collide with another parallel Spec. Apply `requiresWorktree(cwd, canonicalRepo)` (`src/methodology/parallelSlices.ts`): if `cwd` is the canonical repo (or a subdir of it) it returns **`open-worktree`** — **open/redirect into the Spec's worktree session** rather than proceeding in the main tree (the `start_spec_worktree` MCP tool, or the **Start Spec in Worktree** action, does this via `WorktreeService` create-or-reuse + board-root inject, SL-7/SL-8); if `cwd` is already a linked worktree it returns **`proceed`**. Then confirm the branch: in the worktree `spec/SP-{n}` already exists — just confirm you're on it; slices land as commits here, no per-slice branch.
...
5. **Detect an orphaned worktree Spec and offer to resume (recovery).** Before picking fresh work, check whether this Spec is **worktree-shaped but abandoned**: its slices carry an `assignee:` stamp (a worktree/teammate claimed them) and are still open (`doing` / `ready`), yet there is **no live arbiter holder** for them — the signature of a Spec whose worktree session died (a crash or window reload). This is exactly what `detectRecoverable(slices, liveHolders)` (`src/methodology/parallelSlices.ts`) flags, and the ownership map is reconciled board-wins via `reconcileOwnership` (an abandoned holder's files are reclaimed). When detected, surface it: the worktree on disk, the `assignee`-stamped slices, and resume that worktree session rather than starting fresh — the live respawn rides the existing `WorktreeService` + Claude session recovery. Done/archived slices are finished, not orphaned.
6. **Pick the next slice.** Apply this priority:
...
7. **Surface the picked slice.** Show: handle, title, the AC line(s) it satisfies, dependencies satisfied or pending.
8. **Flag stale slices.** If any **done** slice under this Spec has `specChange: "requirements"` — its parent Spec's `## Acceptance Criteria` / `## Design` / `## Constraints` changed since it was verified — surface it prominently: it needs re-verification (handled by `/pair-next`'s stale-spec sweep) before new work proceeds. Ignore `metadata`-only staleness (status/priority/checkbox-toggle — not a real change).
9. **State the mode.** Echo the current `thinkube.kanban.mode` value and what it means:
