...
test("opening two different space keys creates one tab per key and both stay registered", () => {
  ...
  assert.equal((tabA as unknown as { disposed: boolean }).disposed, false, "opening space B never disposes space A's tab");
  assert.equal((tabB as unknown as { disposed: boolean }).disposed, false, "space B's own tab is not disposed by opening it");
});
...
test("opening a key that already has a live tab reveals it and creates no second one", () => {
  ...
  assert.equal((first as unknown as { revealed: number }).revealed, 1, "the already-open tab was revealed on the second open");
});
...
test("a tab that reports itself closed is dropped, so opening that key again creates a fresh tab", () => {
  ...
  assert.equal((second as unknown as { disposed: boolean }).disposed, false, "the fresh tab was not itself disposed on creation");
});
...
test("disposing the register disposes every registered tab and leaves it empty", () => {
  ...
  assert.equal((tabA as unknown as { disposed: boolean }).disposed, true, "the first tab was disposed");
  assert.equal((tabB as unknown as { disposed: boolean }).disposed, true, "the second tab was disposed");
