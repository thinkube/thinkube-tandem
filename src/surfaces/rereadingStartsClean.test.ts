...
test("what the re-read sentences produced goes; what other sentences produced stays", () => {
  ...
  assert.deepEqual(out.specs!.map((sp) => sp.id), ["spec-2"], "the thing around the re-read subject is gone");
  assert.deepEqual(out.questions.map((q) => q.id), ["q-2"], "an undecided question of a re-read sentence goes; a decided one is a decision and stays");
});
