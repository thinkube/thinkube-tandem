...
test("the walk: write, read, keep, group, choose, work out, read again, build, run, accept", async () => {
  ...
  assert.equal(v.strip, "Go on to the next", "the work is in the project already — the press moves on");
  ...
  assert.equal(v.strip, "Go on to the next", "and the one press moves on, not another merge");
