...
export function buildAuditPrompt(
  ...
  return [
    ...
    "for `needs-reframe`.",
    "Every file path inside a `run` command MUST be REPO-RELATIVE from the repo root, exactly as",
    "the toolchain resolves it (e.g. `node --test out-test/scratchpad/clusters.test.js`, whose",
    "source is `src/scratchpad/clusters.test.ts`) — NEVER a bare filename like `clusters.test.ts`:",
    "the runnability gate resolves targets against tsconfig include patterns and a bare filename",
    "resolves to nothing, so the certification is refused at slicing (field defect 2026-08-04).",
  ].join("\n");
}
...
/** Fill `{spec}`/`{ac}`/`{space}` in an acceptance-probe template. `spec` is sanitized to a
 *  path-safe token (any non-`[A-Za-z0-9._-]` run → `_`) so a composite id like `6/3` yields `6_3`.
 *  `{space}` is the thinking-space name — TEP numbers repeat across spaces, so probe identity
 *  needs the space in the filename. Both the auditor (run) and the slicing paths (sourcePath)
 *  fill the same way so the declared path and the written path match. */
export function fillProbeTemplate(
  ...
  const safeSpace = String(space ?? "").replace(/[^A-Za-z0-9._-]+/g, "_");
  return template
    .replace(/\{spec\}/g, safeSpec)
    .replace(/\{ac\}/g, String(ac))
    .replace(/\{space\}/g, safeSpace);
}
