 ...
 * `wrapper/` `cd` to a per-invocation target directory before exec-ing the
 * real CLI, which is what lets "Open Here" actually root a session in the
...
import { ensureStableExtensionLink, stableExtensionSubpath } from "./stablePath";
...
export class LauncherService implements vscode.Disposable {
  ...
  async activate(): Promise<void> {
    ...
    await ensureStableExtensionLink(this.context).catch(() => {});
   ...
   * argument of `claude-vscode.editor.open` lands in the input box).
   * Default: the `[repo/sub]` prefix.
   */
