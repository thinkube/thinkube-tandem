...
export async function driveOne(a: DriveArgs, c: ToDrive, ord: number): Promise<Proof[] & { noticed?: Finding[] }> {
  ...
  const reply = await drive(
    ...
    [
      ...
      "",
      "You have the product to yourself: no other reviewer is working in it",
      "while you are. So anything already named `tandem check ·` is a",
      "leftover of an earlier run — this machinery's own, never a finding",
      "and never evidence about the product. Clear those away before you",
      "judge anything, so the list holds only what you put in it. That is",
      "what makes a criterion about the whole list — nothing is high",
      "priority, there is nothing at all — reachable.",
      "",
      "Everything NOT named `tandem check ·` belongs to a person. Never",
      "delete it, never change it, and never count it as yours. If a",
      "person's own things are still in the way of a criterion that only",
      "holds in an empty product, say so rather than failing it.",
      "",
