...
const KNOWN_MARKETPLACES: Record<
  string,
  { type: "github" | "local"; repo?: string; path?: string }
> = {
  thinkube: {
    type: "local",
...
export class PluginService {
  ...
  async getMarketplaces(
    projectPath?: string,
  ): Promise<{ name: string; source: PluginInfo["source"] }[]> {
  ...
  async getAvailablePlugins(): Promise<
    { plugin: PluginInfo; marketplace: string }[]
  > {
    ...
    try {
      ...
      for (const [pluginKey, enabled] of Object.entries(enabledPlugins)) {
        ...
        if (match) {
          ...
          const pluginInfo = catalog?.plugins.find(
            (p) => p.name === pluginName,
          );
  ...
  async installPlugin(
    ...
    if (!KNOWN_MARKETPLACES[marketplaceName]) {
      ...
      const extras = settings.extraKnownMarketplaces as Array<{
        name: string;
        source: unknown;
      }>;
      if (!extras.find((e) => e.name === marketplaceName)) {
        ...
        console.warn(
          `Marketplace ${marketplaceName} source not known, plugin may not work`,
        );
      }
    }
  ...
  async createPlugin(options: PluginCreationOptions): Promise<string> {
    ...
    if (options.components.skills) {
      fs.mkdirSync(path.join(pluginDir, "skills", `${options.name}-skill`), {
        recursive: true,
      });
    }
...
`,
  ...
  isPluginFolder(folderPath: string): boolean {
    return fs.existsSync(
      path.join(folderPath, ".claude-plugin", "plugin.json"),
    );
  }
   ...
   */
  async suggestPlugins(
    projectPath?: string,
  ): Promise<{ plugin: PluginInfo; marketplace: string; reason: string }[]> {
    const basePath = projectPath || this.basePath;
    const suggestions: {
      plugin: PluginInfo;
      marketplace: string;
    ...
    const isInstalled = (name: string, marketplace: string) =>
      installedPlugins.some(
        (p) => p.name === name && p.marketplace === marketplace,
      );
