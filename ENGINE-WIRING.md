...
| `src/engine/verificationRunnable.ts` | retire | its header names the importers as the `create_slice`→Ready gate in `kanbanMcpServer.ts` and that gate's own `specGateDispatch.test.ts`, neither of which exists in this repository, so wiring means calling from a caller that lives in the separate kanban MCP server product — and its precheck over `tsconfig.test.json`'s `include` has no wired sibling here to fold into, since this repository registers its test sources with a single `["src"]` include that no per-file registration check applies to. |
