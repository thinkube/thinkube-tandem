// AC-9 (TRANSITION): the brief's opening orientation line must still tell
// the worker where its intent lives and that hunting the filesystem for a
// specs directory is pointless — the corrected version says the intent is
// EMBEDDED BELOW (under THE INTENT marker, per AC-3) and that the specs dir
// is NOT IN THIS WORKTREE, instead of routing to a spec/slice block that no
// longer exists. Applies to both roles. Its job is done once dispatch.ts
// supplies this corrected orientation on every brief it builds.
//
...
function tmpRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sl4-ac9-"));
...
test("the brief states the intent is embedded below and the specs dir is not in this worktree, for both roles", async () => {
  const repo = tmpRepo();
  const { space, ids } = spaceWithOneChange();
  const cut = { id: "cut-1", changeIds: ids, tepId: "TEP-sl4-ac9" };
  ...
  for (const b of briefs) {
    assert.match(
      b.text,
      /embedded below/i,
      `${b.role} brief must say the intent is embedded below`,
    );
    assert.match(
      b.text,
      /specs? dir(?:ectory)?[^\n]*not[^\n]*(?:in this worktree|worktree)/i,
      `${b.role} brief must say the specs dir is not in this worktree`,
    );
  }
});
